...
private _condstr = if (!unitIsUAV _this) then {
	"speed _target == 0 && {!isEngineOn _target && {d_player_canu && {player == currentPilot _target && {player inArea 'd_base_marker' || {d_add_resp_points_pos findIf {_x distance2D _target < 700} > -1}}}}}"
} else {
