		...
		if (_x # 1 isEqualType "") then {
			if (markerPos (_x # 1) isNotEqualTo [0,0,0]) then {
				...
				if (d_player_side != _x # 3) then {
					...
					_dadao addAction [format ["<t color='#4169E1' size='1.5'>%1</t>", localize "STR_DOM_MISSIONSTRING_533"], {call d_fnc_teleportx}, -1, 1.5, true, true, "", "true", 10];
					if (d_ParaAtBase == 0) then {
						_dadao setVariable ["d_jf_id", _dadao addAction [format ["<t color='#4169E1' size='1.2'>%1</t>", localize "STR_DOM_MISSIONSTRING_296"], {_this spawn d_fnc_paraj}, 0, 1.5, true, true, "", "true", 10]];
					};
					if (count _x > 4 && {_x # 4}) then {
						_dadao addAction [format ["<t color='#7F7F7F'>%1</t>", localize "STR_DOM_MISSIONSTRING_1816"], {[0, 0, 0, [d_create_bike # 0, 1]] spawn d_fnc_bike}, -1, 1.5, true, true, "", "true", 10];
					};
				};
#else
				private _dadao = missionNamespace getVariable (_x # 1);
				...
				if (isNil {_dadao getVariable "d_teleidx"}) then {
					_dadao setVariable ["d_teleidx", _dadao addAction [format ["<t color='#4169E1' size='1.5'>%1</t>", localize "STR_DOM_MISSIONSTRING_533"], {call d_fnc_teleportx}, -1, 1.5, true, true, "", "true", 10]];
				};
				if (d_ParaAtBase == 0) then {
