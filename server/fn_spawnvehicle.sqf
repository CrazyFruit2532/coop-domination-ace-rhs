...
if (_sim in ["airplane", "helicopter", "airplanex", "helicopterx", "helicopterrtd"]) then {
	...
	if (_pos_flat_empty isEqualTo []) then {
		diag_log ["fn_spawnvehicle could not find a flat and empty spot after 99 attempts, giving up and using original position, vtype:", _typev1, "Original position:", _posv1];
